...
class AveragePrecisionMeter(object):
    ...
    def evaluation(self, scores_, targets_):
        ...
        Np[Np == 0] = 1  # Prevent division by zero
        Ng[Ng == 0] = 1  # Prevent division by zero
    
        OP = np.sum(Nc) / np.sum(Np)
        OR = np.sum(Nc) / np.sum(Ng)
        if OP + OR == 0:  # Prevent division by zero in OF1 calculation
            OF1 = 0
        else:
            OF1 = (2 * OP * OR) / (OP + OR)
        ...
        if CP + CR == 0:  # Prevent division by zero in CF1 calculation
            CF1 = 0
        else:
            CF1 = (2 * CP * CR) / (CP + CR)
